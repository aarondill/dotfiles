...
BIG_SWAP_LABEL=$(sudo_cmd blkid -s LABEL -o value "$BIG_SWAP_PATH" 2>/dev/null || printf '')
# fallback to parition label
[ -z "$BIG_SWAP_LABEL" ] && BIG_SWAP_LABEL="$(sudo_cmd blkid -o value -s PARTLABEL "$BIG_SWAP_PATH" 2>/dev/null || printf '')"
[ -z "$BIG_SWAP_LABEL" ] && BIG_SWAP_LABEL="No Label"

BIG_SWAP_UUID="$(sudo_cmd blkid -o value -s UUID "$BIG_SWAP_PATH")"
...
if [ -n "$INITRAMFS_RESUME_FILE" ]; then
  cat <<EOF | sudo_cmd tee "$INITRAMFS_RESUME_FILE" >/dev/null
...
cat <<EOF | sudo_cmd tee "$GRUB_RESUME_FILE" >/dev/null
#!/usr/bin/env sh
# THIS FILE IS AUTOGENERATED. Edits will likely be lost.
